...
export class StreamSession extends EventEmitter {
  ...
  constructor(public readonly config: StreamSessionConfig) {
    ...
    // pipe relies on the renderer's catch-up loop (see renderer.worker.ts,
    // renderLoop) because re-feeding an encoded P-frame would corrupt the bitstream.
  ...
  /**
   * Stop and resolve once ffmpeg has actually exited, bounded by a hard timeout
   * so shutdown can't hang. stop() escalates to SIGKILL at 5s if SIGINT is
   * ignored (common for ffmpeg on Windows).
   */
  public async stopAndWait(timeoutMs = 8000): Promise<void> {
