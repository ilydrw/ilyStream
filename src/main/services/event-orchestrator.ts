...
export class EventOrchestrator {
  ...
  private handleHardwareAlerts(event: any): void {
    ...
    const hueSettings = settings.integrations?.hue
    const goveeSettings = settings.integrations?.govee
    const hueFlashDurationMs = hueSettings?.flashDurationMs ?? (settings as any).hueFlashDurationMs
    const goveeFlashDurationMs = goveeSettings?.flashDurationMs ?? (settings as any).goveeFlashDurationMs
    const hueFlashOnFollow = hueSettings?.flashOnFollow ?? (settings as any).hueFlashOnFollow
    const goveeFlashOnFollow = goveeSettings?.flashOnFollow ?? (settings as any).goveeFlashOnFollow
    const hueFlashOnGift = hueSettings?.flashOnGift ?? (settings as any).hueFlashOnGift
    const goveeFlashOnGift = goveeSettings?.flashOnGift ?? (settings as any).goveeFlashOnGift

    if (event.type === 'follow') {
      const duration = hueFlashDurationMs || goveeFlashDurationMs || 5000
      for (const device of devices) {
        if (device.platform === 'hue' && hueFlashOnFollow) {
          this.lightingManager.executeAction(device.id, 'flash', { duration })
        } else if (device.platform === 'govee' && goveeFlashOnFollow) {
          this.lightingManager.executeAction(device.id, 'flash', { duration })
    ...
    if (event.type === 'gift' && !event.isCombo) {
      if (this.lowValueGiftHardwareCooldown.shouldSuppress(event)) {
        console.log('[orchestrator] Suppressed low-value TikTok gift lighting during anti-spam cooldown.')
        return
      }

      const duration = hueFlashDurationMs || goveeFlashDurationMs || 5000
      for (const device of devices) {
        if (device.platform === 'hue' && hueFlashOnGift) {
          this.lightingManager.executeAction(device.id, 'pulse', { duration, color: '#D035F1' })
        } else if (device.platform === 'govee' && goveeFlashOnGift) {
          this.lightingManager.executeAction(device.id, 'pulse', { duration, color: '#D035F1' })
...
function shouldRouteEventToTts(event: AnyStreamEvent): boolean {
  if (shouldSuppressStreamEventFromChat(event)) return false
...
function isDisplayOnlyChatEvent(event: AnyStreamEvent): boolean {
  return event.type === 'chat' && (
    shouldSuppressStreamEventFromChat(event) ||
