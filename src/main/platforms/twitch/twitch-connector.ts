...
export class TwitchConnector extends BaseConnector {
  ...
  private async backfillMissingUserIds(): Promise<void> {
    if (!this.apiClient || !this.db) return
    try {
      const usernames = this.db.stats.getTwitchUsernamesWithNullId()
      if (usernames.length === 0) return

      console.log(`[twitch-connector] Found ${usernames.length} Twitch users with missing user IDs. Fetching from Helix...`)

      for (let i = 0; i < usernames.length; i += 100) {
        const chunk = usernames.slice(i, i + 100)
        const twitchUsers = await this.apiClient.users.getUsersByNames(chunk)

        for (const twitchUser of twitchUsers) {
          const username = twitchUser.name.toLowerCase()
          const platformUserId = twitchUser.id
          const displayName = twitchUser.displayName
          const profilePictureUrl = this.readTwitchProfilePictureUrl(twitchUser) || null

          this.db.stats.updatePlatformUserId('twitch', username, platformUserId, {
            displayName,
            profilePictureUrl
          })
        }
      }
      console.log('[twitch-connector] Finished backfilling missing Twitch user IDs.')
    } catch (err) {
      console.warn('[twitch-connector] Failed to backfill missing Twitch user IDs:', err)
    }
  }
