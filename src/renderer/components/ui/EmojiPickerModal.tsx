  ...
  return (
    ...
    >
      <div className="p-8 space-y-8">
        <div className="flex items-center gap-6 p-4 rounded-md bg-white/[0.02] border border-white/5">
          ...
          <div className="flex-1">
            <p className="text-[10px] font-semibold text-white/20 tracking-normal mb-1">Preview</p>
            <p className="text-sm font-semibold text-white truncate">{assetName || 'Unnamed Asset'}</p>
          </div>
        </div>

        <div className="space-y-6">
          <div className="space-y-3">
            <label className="text-[10px] font-semibold tracking-tight text-white/20 px-1">Display Name</label>
            <input
              type="text"
              value={assetName}
              onChange={(e) => setAssetName(e.target.value)}
              placeholder="Enter asset name..."
              className="w-full bg-black/40 border border-white/5 rounded-xl px-5 h-14 text-sm text-white focus:border-accent/40 focus:bg-black/60 transition-all outline-none font-semibold"
            />
          </div>

          {activeCategory !== '__rename_only__' && (
            <div className="space-y-4">
              <label className="text-[10px] font-semibold tracking-tight text-white/20 px-1">Associated Icon</label>
              ...
              {!isSearching && (
                <div className="grid grid-cols-9 gap-1.5 p-1 bg-white/[0.02] rounded-xl border border-white/5">
                  <button
                    ...
                    type="button"
                    className={`h-9 rounded-lg flex items-center justify-center transition-all cursor-pointer ${ activeCategory === RECENTS_KEY ? 'bg-accent text-black ' : 'text-white/40 hover:bg-white/5 hover:text-white' }`}
                  >
                    <IconClock size={16} />
                  </button>
                  {EMOJI_CATEGORIES.map((category) => (
                    <button
                      ...
                      type="button"
                      className={`h-9 rounded-lg flex items-center justify-center text-base transition-all cursor-pointer ${ activeCategory === category.id ? 'bg-accent text-black ' : 'text-white/60 hover:bg-white/5' }`}
                    >
                      {category.icon}
                    </button>
                  ))}
              ...
              >
                {visibleEmojis.map((emoji, idx) => (
                  <button
                    ...
                    className={`text-2xl p-2 rounded-lg hover:bg-accent/10 hover:scale-110 transition-all cursor-pointer flex items-center justify-center ${ emojiInput === emoji ? 'bg-accent/20 border border-accent/40 ' : 'border border-transparent' }`}
                  >
                    {emoji}
                  </button>
                ))}
                ...
                {visibleEmojis.length === 0 && (
                  <div className="col-span-full flex flex-col items-center justify-center h-full text-white/20 space-y-2 py-8">
                    ...
                    <p className="text-xs font-semibold tracking-tight">No emojis found</p>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="flex gap-3 pt-4 border-t border-white/5">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 h-12 rounded-xl bg-white/5 hover:bg-white/10 text-white/70 font-semibold text-xs tracking-tight transition-all cursor-pointer"
          >
            Cancel
          </button>
          <button
            ...
            className="flex-[2] h-12 rounded-xl bg-accent text-black font-semibold text-xs tracking-normal transition-all cursor-pointer disabled:opacity-20 disabled:grayscale hover: hover:scale-[1.02] active:scale-[0.98]"
          >
