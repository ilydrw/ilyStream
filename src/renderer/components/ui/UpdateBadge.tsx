import { IconArrowDownCircle, IconLoader2 } from '@tabler/icons-react'
  ...
  if (updateStatus.state === 'download-progress') {
    return (
      <Tooltip content={`Downloading v${updateStatus.version}...`} position="bottom">
        <div className="flex items-center gap-2 px-3 py-1.5 bg-white/5 border border-white/10 text-white/60 font-semibold text-[11px] tracking-wider rounded-lg">
  ...
  if (updateStatus.state === 'checking' || updateStatus.state === 'available') {
    return (
      <Tooltip content="Connecting to update server..." position="bottom">
        <div className="flex items-center gap-2 px-3 py-1.5 bg-white/5 border border-white/10 text-white/40 font-semibold text-[11px] tracking-wider rounded-lg">
