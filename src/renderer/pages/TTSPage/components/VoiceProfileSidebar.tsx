      ...
      <div className="app-section-content">
        ...
        {profiles.map((profile) => (
          <button
            ...
            className={`flex items-center gap-4 w-full px-2 py-3 rounded-xl transition-all group ${ selectedProfileId === profile.id ? 'bg-white/[0.05] text-white' : 'bg-transparent text-white/20 hover:bg-white/[0.02] hover:text-white/40' }`}
          >
            ...
            <div className="flex-1 text-left min-w-0">
              ...
              <span className="text-[9px] font-semibold tracking-normal opacity-20">{profile.provider}</span>
            </div>
            {profile.isDefault && (
              <span className="text-[10px] font-semibold tracking-tighter bg-white/10 px-2 py-0.5 rounded-full">Default</span>
            )}
          </button>
        ))}
      </div>
