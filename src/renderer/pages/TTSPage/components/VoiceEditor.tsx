              ...
              <div className="space-y-8">
                <div className="space-y-3">
                  <div className="flex justify-between text-xs font-semibold tracking-tight text-white/60">
                    <span>Pitch</span>
                    <span className="text-accent">{(draft.pitch ?? 1).toFixed(2)}x</span>
                  </div>
                  <input
                    type="range" min="0.5" max="2" step="0.05"
                    value={draft.pitch ?? 1}
                    onChange={(e) => onUpdateDraft({ pitch: parseFloat(e.target.value) })}
                ...
                {provider === 'elevenlabs' && (
                  <div className="space-y-5 rounded-md border border-white/5 bg-white/[0.015] p-4">
                    <div className="flex items-center justify-between">
                      <label className="text-xs font-semibold tracking-tight text-white/40">ElevenLabs Voice Settings</label>
                      <span className="text-[10px] font-medium text-white/25">sent with every generation</span>
                    </div>
                    <ElevenLabsRange
                      label="Stability"
                      value={draft.elevenlabsStability ?? 0.5}
                      onChange={(value) => onUpdateDraft({ elevenlabsStability: value })}
                    />
                    <ElevenLabsRange
                      label="Similarity"
                      value={draft.elevenlabsSimilarity ?? 0.8}
                      onChange={(value) => onUpdateDraft({ elevenlabsSimilarity: value })}
                    />
                    <ElevenLabsRange
                      label="Style"
                      value={draft.elevenlabsStyle ?? 0}
                      onChange={(value) => onUpdateDraft({ elevenlabsStyle: value })}
                    />
                  </div>
                )}
              </div>
...
function ElevenLabsRange({
  ...
  return (
    <div className="space-y-2">
      <div className="flex justify-between text-[11px] font-semibold tracking-tight text-white/55">
        <span>{label}</span>
        <span className="text-accent">{Math.round(value * 100)}%</span>
      </div>
      <input
        type="range"
        min="0"
