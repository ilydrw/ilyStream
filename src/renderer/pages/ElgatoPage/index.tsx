...
import { IconCpu, IconRadio, IconDeviceDesktop, IconBulb, IconKeyboard } from '@tabler/icons-react'
...
function Metric({ icon, label, value, sub, accent = 'text-accent' }: { icon: any; label: string; value: string; sub: string; accent?: string }) {
  return (
    <div className="app-section-card glass !p-6 hover:border-white/10 transition-all group">
      ...
      <div className="text-[9px] font-semibold text-white/10 tracking-wider">{sub}</div>
    </div>
  )
}
    ...
    <div className="app-page">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <section>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-semibold flex items-center gap-3">
                Discovered Hardware
                <span className="px-2 py-0.5 rounded-md bg-white/5 text-[10px] font-semibold text-white/40">
                  {devices.length}
                </span>
              </h2>
              <button
                ...
                className="flex items-center gap-2 !h-10 px-4 rounded-xl bg-white/5 hover:bg-white/10 text-[10px] font-semibold tracking-tight transition-all disabled:opacity-50"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {devices.length > 0 ? (
                devices.map(device => (
                  <div key={device.id} className="bg-white/[0.02] border border-white/5 p-5 rounded-md hover:border-white/10 transition-all group">
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <div className={`px-2 py-1 rounded-lg text-[10px] font-semibold tracking-wider ${ device.status === 'connected' ? 'bg-success/10 text-success' : 'bg-white/5 text-white/20' }`}>
                        {device.status}
                      </div>
                    </div>
                    <div>
                ...
                <div className="col-span-2 border border-dashed border-white/5 rounded-md p-10 flex flex-col items-center justify-center text-center">
                  <div className="w-16 h-16 rounded-full bg-white/5 flex items-center justify-center text-white/10 mb-4">
                    <IconCpu size={32} />
                  </div>
                  <h3 className="font-semibold text-white/40 mb-1">No Hardware Found</h3>
                  <p className="text-xs text-white/20 max-w-[240px]">
                    Ensure your Elgato software is running and devices are on the same network.
                  </p>
                </div>
              )}
              <button className="border-2 border-dashed border-white/5 rounded-md p-5 flex flex-col items-center justify-center gap-2 hover:border-accent/30 hover:bg-accent/5 transition-all text-white/20 hover:text-accent">
                <IconPlus size={24} />
                <span className="text-[10px] font-semibold tracking-tight">Add Manual IP</span>
              </button>
            </div>
          </section>
        </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-gradient-to-br from-accent/20 to-transparent border border-accent/20 p-6 rounded-lg relative overflow-hidden group">
            <div className="relative z-10">
              <h3 className="text-lg font-semibold mb-2">Production Scenes</h3>
              <p className="text-xs text-white/60 mb-6 leading-relaxed">
                Create macros that control multiple Elgato devices at once. Sync your Key Lights with your Prompter script.
              </p>
              <button className="w-full py-3 rounded-md bg-accent text-white font-semibold text-xs tracking-tight hover:scale-[1.02] transition-all">
                Configure Scenes
              </button>
            </div>
          ...
          <div className="bg-white/[0.03] border border-white/5 p-6 rounded-lg">
            ...
            <div className="mt-8 pt-6 border-t border-white/5">
              <button className="flex items-center gap-2 text-[10px] font-semibold tracking-tight text-white/20 hover:text-white transition-colors">
