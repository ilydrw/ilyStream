  ...
  return (
    <div className="flex items-center justify-between p-4 rounded-lg bg-white/[0.015] border border-white/5 hover:border-white/10 hover:bg-white/[0.02] transition-all group animate-in fade-in slide-in-from-bottom-1 duration-500">
      <div className="flex items-center gap-4 min-w-0 flex-1">
        <div className="w-12 h-12 rounded-md bg-white/[0.03] border border-white/5 flex items-center justify-center shrink-0 transition-all">
          {sound.emoji ? (
            <span
              className={`text-2xl select-none transition-transform duration-300 ${ isPreviewing ? 'scale-110' : '' }`}
            >
              {sound.emoji}
            </span>
          ) : (
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-[9px] font-semibold tracking-tight text-white/10 mt-1">Audio Module</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        <button
          onClick={handlePlay}
          className={`app-button !h-10 !w-10 !p-0 ${ isPreviewing ? '!text-accent !bg-accent/10' : '!bg-white/[0.03] !text-white/40 hover:!text-white' }`}
          title="Preview sound"
        >
