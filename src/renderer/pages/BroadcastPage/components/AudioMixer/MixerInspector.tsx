        ...
        <div className="p-6 border-b border-white/[0.06] bg-black/20">
          ...
          {selectedSource.id !== 'master' && (
            <button
              ...
              className="mt-4 h-11 w-full rounded-xl border border-red-500/20 bg-red-500/5 text-red-400/60 hover:bg-red-500/10 hover:text-red-400 hover:border-red-500/30 transition-all flex items-center justify-center gap-2.5 text-xs font-medium tracking-normal"
        ...
        <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar p-5 space-y-5">
          <section>
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-2xs font-semibold tracking-normal text-white/22">Master bus</span>
            </div>
            {selectedSource.id !== 'master' && (
              <div className="mb-3 grid grid-cols-2 gap-2 rounded-xl bg-white/[0.025] p-1 ring-1 ring-white/[0.06]">
                {(['mono', 'stereo'] as const).map(mode => (
                  <button
                    ...
                    className={`h-9 rounded-lg text-xs font-semibold tracking-tight transition-all ${ sanitizeChannelMode(selectedSource.channelMode, selectedSource.type === 'mic' ? 'mono' : 'stereo') === mode ? 'bg-accent text-white ' : 'text-white/28 hover:bg-white/[0.035] hover:text-white/55' }`}
                  >
                    {mode}
                  </button>
                ))}
          ...
          <section>
            <div className="flex items-center justify-between mb-3">
              ...
              <select
                ...
                className="h-8 rounded-lg bg-white/[0.04] border border-white/[0.07] px-2 text-xs font-semibold text-white/50 outline-none [&>option]:bg-[#1a1a1a]"
              >
            ...
            <div className="space-y-3">
              {(selectedSource.filters || []).length === 0 ? (
                <div className="h-28 rounded-xl border border-dashed border-white/[0.08] bg-white/[0.015] flex flex-col items-center justify-center gap-2 text-white/20">
                  <IconPlus size={18} />
                  <span className="text-xs font-semibold tracking-normal">No inserts loaded</span>
                </div>
              ) : (
...
function HeaderLabel({ icon: Icon, label }: { icon: any; label: string }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-[10px] font-medium tracking-normal text-white/40">{label}</span>
    </div>
  )
}
