...
static const bgfx::EmbeddedShader s_blurEmbeddedShaders[] = {
    BGFX_EMBEDDED_SHADER(vs_sprite),
...
bgfx::ProgramHandle CreateBlurProgram() {
    const bgfx::RendererType::Enum type = bgfx::getRendererType();
    bgfx::ShaderHandle vertex = bgfx::createEmbeddedShader(s_blurEmbeddedShaders, type, "vs_sprite");
    bgfx::ShaderHandle fragment = bgfx::createEmbeddedShader(s_blurEmbeddedShaders, type, "fs_blur");
    if (!bgfx::isValid(vertex) || !bgfx::isValid(fragment)) {
        if (bgfx::isValid(vertex)) bgfx::destroy(vertex);
        if (bgfx::isValid(fragment)) bgfx::destroy(fragment);
        return BGFX_INVALID_HANDLE;
