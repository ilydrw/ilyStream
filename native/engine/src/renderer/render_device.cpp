...
IlyResult RenderDevice::DrawQuad(ResourceHandle textureHandle, const IlyTransform& transform, float opacity, IlyBlendMode blendMode, const IlyChromaKey* chroma, const IlyColorAdjust* colorAdjust, float cornerRadius, float blurSigma, const IlyCircleMask* circleMask, ResourceHandle maskTexture, const float* maskTransform) {
    ILY_PROFILE_SCOPE("RenderDevice::DrawQuad");
    std::lock_guard<std::mutex> lock(m_mutex);
    if (!m_initialized) return ILY_ERROR_INITIALIZATION_FAILED;
    return m_backend->DrawQuad(textureHandle, transform, opacity, blendMode, chroma, colorAdjust, cornerRadius, blurSigma, circleMask, maskTexture, maskTransform);
}
